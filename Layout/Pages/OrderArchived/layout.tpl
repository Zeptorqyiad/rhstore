...
App\Layout\Components\HeaderBlock\Header\Layout::draw(); ?>
    ...
    <main class="order-archived">
        <div class="order-archived__back">
            <div class="order-archived__wrap">
                <?php
                App\Layout\Components\Common\NavAside\Layout::draw([
                    'class_name' => 'order-archived__aside',
                    'active' => '3'
                ]); ?>

                <div class="order-archived__column">
                    <?php
                    App\Layout\Components\Common\BreadCrumbs\Layout::draw([
                        'class_name' => 'order-archived__breadcrumbs',
                        'text' => 'Архивные заказы'
                    ]); ?>

                    <div class="order-archived__body">
                        <div class="order-archived__details">
                            <h3 class="order-archived__number">Заказ <?= $data['number'] ?></h3>
                            <p class="order-archived__date"><?= $data['date'] ?></p>
                            <div class="order-archived__status order-archived__status_<?= $data['badge_color'] ?>"><?= $data['status'] ?></div>
                            <a href="<?= $data['track_link'] ?>" class="order-archived__link">Отслеживание заказа</a>
                            <div class="order-archived__sum">Сумма: <?= $data['sum'] ?> ₽</div>
                            <div class="order-archived__count">Товаров: <?= $data['count'] ?> шт.</div>

                            <?php
                            if ($data['is_repeat'] === true): ?>
                                <button class="order-archived__repeat">
                            ...
                            <?php elseif (!$archived): ?>
                                <?php App\Layout\Components\Common\Button\Layout::draw([
                                    'text' => 'Отменить',
                                    'style' => 'gray',
                                    'class_name' => 'order-archived__cancel',
                                    'link' => '',
                                ]); ?>
                            <?php endif; ?>
                        </div>
                        ...
                        if ($docs = json_decode($data['order']->documents, true)['v'] ?? []): ?>
                            <div class="order-archived__documents">
                                <h3 class="order-archived__title">Документы</h3>
                                <ul class="order-archived__doc-list">
                                    <?php
                                    foreach ($docs as $item): ?>
                                        <li class="order-archived__doc-item">
                                            <p class="order-archived__doc-item-name"><?= $item['name'] ?></p>
                                            <p class="order-archived__doc-item-date"><?= $item['date'] ?></p>
                                            <p class="order-archived__doc-item-price"><?= $item['sum'] ?></p>
                                            <?php
                                            App\Layout\UIKit\ButtonDownload\Layout::draw([
                                                'class_name' => 'order-archived__doc-item-download',
                                                'link' => $item['file'],
                                            ]); ?>
                                        </li>
                                    <?php
                                ...
                                <button class="order-archived__show-more">
                        ...
                        <div class="order-archived__cards-wrapper">
                            <div class="order-archived__cards-wrapper">
                                <div class="order-archived__sort-wrap">
                                    <form>
                                        <?php
                                    ...
                                    <button class="order-archived__filters-button">
                                        <svg class="order-archived__filters-icon"
                                ...
                                \App\Layout\Components\Blocks\Catalog\CatalogHeadlines\Layout::draw([
                                    'class_name' => 'order-archived__headlines'
                                ]); ?>
                                <ul class="order-archived__cards">
                                    <?php
                                    ...
                                    foreach ($ci as $pi):
                                        ...
                                        <li class="order-archived__card">
                                            ...
                                            \App\Layout\Components\Blocks\Catalog\CatalogCard\Layout::draw([
                                                ...
                                                'class_name' => 'order-archived__catalog-card',
                                                'id' => $prod->product_id,
                                ...
                                <ul class="order-archived__mobile-cards">
                                    <?php
                                    foreach ($ci as $pi): ?>
                                        <?php
                                        ...
                                        App\Layout\Components\Common\ProductCard\Layout::draw([
                                            ...
                                            'fav' => $prod->inFav(),
                                            'cartc' => $pi['qty'],
                                            'class_name' => 'order-archived__product-card',
                                            'horizontal' => true,
                                        ]);
                                        ?>
                                    <?php
