...
<div class="<?= $data["class_name"] ?> order-cards">
    <ul class="order-cards__list">
        <?php
        foreach ($data['cards'] as $card): ?>

            <li class="order-cards__card" data-id="<?= $card['number'] ?>">
                <h3 class="order-cards__number">
                    Заказ <?= $card['number'] ?>
                </h3>
                <p class="order-cards__date">
                    <?= $card['date'] ?>
                </p>
                <div class="order-cards__status order-cards__status_<?= $card['badge_color'] ?>">
                    <?= $card['status'] ?>
                </div>
                <?php
                App\Layout\Components\Common\Button\Layout::draw([
                    ...
                    'style' => 'secondary',
                    'icon' => true,
                    'class_name' => 'order-cards__button',
                    'link' => $card['link'],
                ]); ?>

                <?php
                if ($data['track_link']): ?>
                    <a href="<?= $data['track_link'] ?>" class="order-cards__link">
                        Отслеживание заказа
                    </a>
                <?php
                endif; ?>

                <div class="order-cards__sum">
                    Сумма: <?= $card['sum'] ?> ₽
                </div>
                <div class="order-cards__count">
                    Товаров: <?= $card['count'] ?> шт.
                </div>

                <?php
                if ($card['is_repeat'] === true): ?>
                    <button class="order-cards__repeat">
                ...
                <?php if (!($card['archived'] || $card['status'] !== 'Ожидает оплаты')): ?>
                    <?php
                    App\Layout\Components\Common\Button\Layout::draw([
                        'text' => 'Отменить',
                        'style' => 'gray',
                        'class_name' => 'order-cards__cancel',
                        'link' => '',
                    ]); ?>
                <?php endif; ?>
