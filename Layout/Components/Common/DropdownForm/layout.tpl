...
<div class="<?= $data["class_name"] ?> dropdown-form" <?= $data['ns'] ? 'data-ns="true"' : '' ?>>
    <label class="dropdown-form__label">
        <input class="dropdown-form__input" type="text" value="<?= $data['items'][$data['value'] ?? ''] ?? '' ?>"
    ...
    <div class="dropdown-form__content">
        <ul class="dropdown-form__options">
            <?php foreach ($data['items'] as $k => $i): ?>
                <li data-value="<?= $k ?>"
                    class="dropdown-form__option"
                >
                    <?= is_array($i) ? $i[0] : $i ?>
                </li>
            <?php endforeach; ?>
